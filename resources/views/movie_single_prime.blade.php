        ...
        @if(isset($movie))
          ...
          <div class="row">
            <div class="col-md-8">
              ...
              @if($subscribed == 1 && $auth)
                @if($movie->maturity_rating =='all age' || $age>=str_replace('+', '',$movie->maturity_rating))
                    @if($movie->video_link['iframeurl'] != null)
                        ...
                        <a href="#" onclick="playoniframe('{{ $movie->video_link['iframeurl'] }}','{{ $movie->id }}','movie')" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                       </a>
                ...
                @endif
                 ...
                 @if($movie->trailer_url != null || $movie->trailer_url != '')
                              <a href="{{ route('watchTrailer',$movie->id) }}" class="iframe btn-default">{{__('staticwords.watchtrailer')}}</a>
                            @endif
             ...
             @else
               @if($movie->trailer_url != null || $movie->trailer_url != '')
                <a href="{{ route('guestwatchtrailer',$movie->id) }}" class="iframe btn-default">{{__('staticwords.watchtrailer')}}</a>
               @endif
            ...
            @if(isset($movie->multilinks) && count($movie->multilinks) > 0 )   
              ...
              @if(in_array(1, $mlc))
                 <a type="button" class="btn btn-default" data-toggle="collapse" data-target="#downloadmovie">{{__('staticwords.download')}}</a>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7 pad-0">
                    ...
                    <div class="des-btn-block des-in-list">
                      <a href="{{route('watchmovie',$single_series->id)}}" class="iframe btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span></a>
                      @if($single_series->trailer_url != null || $single_series->trailer_url != '')
                        <a href="{{route('watchTrailer',$single_series->id)}}" class="btn-default">{{__('staticwords.watchtrailer')}}</a>
                      @endif
                      @if (isset($wishlist_check->added))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7 pad-0">
                    ...
                    <div class="des-btn-block des-in-list">
                      ...
                      <a onclick="playVideo({{$series->id}}, '{{$series->type}}')" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span></a>
                      @else
                      ...
                      @if($series->trailer_url != null || $series->trailer_url != '')
                        <a onclick="playTrailer('{{$series->trailer_url}}')" class="btn-default">{{__('staticwords.watchtrailer')}}</a>
                      @endif
                      @if (isset($wishlist_check->added))
                      ...
                      <div id="prime-mix-description-block{{$item->id}}" class="prime-description-block">
                        ...
                        <div class="des-btn-block">
                           ...
                           @if($item->video_link['iframeurl'] != null)
                              ...
                              <a onclick="playoniframe('{{ $item->video_link['iframeurl'] }}','{{ $item->id }}','tv')" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                              </a>
                    ...
                    <div class="genre-prime-slide">
                      ...
                      <div id="prime-mix-description-block{{$item->id}}{{$item->type}}" class="prime-description-block">
                        ...
                        <div class="des-btn-block">
                          ...
                          @endif
                          @if (isset($wishlist_check->added))
                            <a onclick="addWish({{$item->id}},'{{$item->type}}')" class="addwishlistbtn{{$item->id}}{{$item->type}} btn-default">{{$wishlist_check->added == 1 ? __('removefromwatchlist') : __('addtowatchlist')}}</a>
                          @else
                            <a onclick="addWish({{$item->id}},'{{$item->type}}')" class="addwishlistbtn{{$item->id}}{{$item->type}} btn-default">{{__('staticwords.addtowatchlist')}}</a>
                          @endif
                        </div>
                    ...
                    @foreach($all as $key => $item)
                      @if($item->type == 'S')
                        <div class="genre-slide">
                          ...
                          <div class="genre-slide-dtl">
                            <h5 class="genre-dtl-heading">@if($auth && $subscribed == 1)<a href="{{url('show/detail/'.$item->id)}}">{{$item->tvseries->title}}</a>
                            @else
                      ...
                      @elseif($item->type == 'M')
                        <div class="genre-slide">
                          ...
                          <div class="genre-slide-dtl">
                            <h5 class="genre-dtl-heading">@if($auth && $subscribed == 1)<a href="{{url('movie/detail/'.$item->id)}}">{{$item->title}}</a>
                            @else
