                    ...
                    @if($auth && $subscribed==1)
                      ...
                      @else
                       <a href="{{url('movie/guest/detail', $slide->movie->id)}}">
                        @if ($slide->slide_image != null)
                          <img src="{{url('images/home_slider/movies/'. $slide->slide_image)}}" class="img-responsive" alt="slider-image">
                        @elseif ($slide->movie->poster != null)
                          <img src="{{url('images/movies/posters/'. $slide->movie->poster)}}" class="img-responsive" alt="slider-image">
                        @endif
                      </a>
                      @endif
  ...
  @foreach($menu->menusections as $section)
        @php
            ...
            foreach ($recent_data as $key => $item) {
                ...
                $rm =  \DB::table('movies')
                             ->select('movies.id as id','movies.title as title','movies.type as type','movies.status as status','movies.genre_id as genre_id','movies.thumbnail as thumbnail','movies.live as live')
            ...
            foreach ($recent_data as $key => $item) {
                ...
                $rectvs =  \DB::table('tv_series')
                              ...
                              ->select('seasons.id as seasonid','tv_series.genre_id as genre_id','tv_series.id as id','tv_series.type as type','tv_series.status as status','tv_series.title as title','tv_series.thumbnail as thumbnail')
                              ...
                              @if($item->status == 1)
                                ...
                                @elseif($item->type == 'T')
                                   ...
                                   <div class="genre-slide">
                                      ...
                                      <div class="genre-slide-dtl">
                                         @if($auth && $subscribed==1)
                                          <h5 class="genre-dtl-heading"><a href="{{url('show/detail/'.$item->id)}}">{{$item->title}}</a></h5>
                                          @else
                                           <h5 class="genre-dtl-heading"><a href="{{url('show/guest/detail/'.$item->id)}}">{{$item->title}}</a></h5>
                                          @endif
                                      </div>  
                                   </div>
                                @endif
        ...
        @php
            ...
            foreach ($menu_data as $key => $item) {
                ...
                $fmovie =  \DB::table('movies')
                             ->select('movies.id as id','movies.title as title','movies.type as type','movies.status as status','movies.genre_id as genre_id','movies.thumbnail as thumbnail')
                    ...
                    @if($item->type == 'M')
                      ...
                      @endphp
                              ...
                              <div class="row">
                                 <div class="des-btn-block des-in-list">
                                    @if($auth && $subscribed==1)
                                     ...
                                     @endif
                                    @if($withlogin==0 && $auth)
                                      <a class="iframe btn btn-default" href="{{url('movie/detail',$item->id)}}"><i class="fa fa-info-circle" style="background-color: black;border:1px solid white;"></i> {{__('staticwords.moreinfo')}}</a>
                                    @else
                        ...
                        <div class="col-md-6">
                          ...
                          <p>{{str_limit($item->detail,150)}}</p>
                          <br/>
                          <p class="text-right promotion" style=""># {{count($homeblocks)}}  {{__('staticwords.topin')}} {{$menu->name}}</p>
                        </div>
                      </div>
                    @endif
                  @endif  
